...
async function loadMembership(user){const email=normalizedEmail(user);const ref=firestore.collection("access_users").doc(email);if(email===INITIAL_OWNER_EMAIL){try{await ref.set({email,role:"owner",active:true,created_at:new Date().toISOString()},{merge:true});}catch(error){console.warn("Owner bootstrap skipped:",error.message);}}try{const snap=await ref.get();return snap.exists&&snap.data().active?snap.data():null;}catch(error){return null;}}
